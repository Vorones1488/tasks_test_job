...
async def add_task(request: TaskBaseSchemas):
    ...
    return result
@router.get('/{id:int}', response_model= TaskResponseSchemas, summary='получение задачи по его id', description='получение задачи по ее id')
async def get_task_id(id: int):
    result = await task_services.get_task_id(id)
    return result

@router.get('/', response_model=List[TaskResponseSchemas],summary='получение всех задач', description='получение всех задач с возосжностью фильтрации по статусу')
